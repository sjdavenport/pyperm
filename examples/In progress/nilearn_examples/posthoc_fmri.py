...
def ARI_with_TDP_output(stat_img, mask_img=None,
    ...
    for threshold_ in sorted(threshold):
        ...
        for label_ in range(1, n_labels + 1):
            ...
            proportion = _true_positive_fraction(cluster_vals, len(stats),
                                                 alpha)  # = SansSouci
            # proportion = _true_positive_fraction(cluster_vals, hommel_value,
            # alpha)
            proportion_true_discoveries[labels == label_] = proportion
            proportion_per_cluster.append(proportion)

    proportion_true_discoveries_img = masker.inverse_transform(
        proportion_true_discoveries)
    return proportion_true_discoveries_img, proportion_per_cluster


def ARI_with_ROI_input(stat_img, cluster, mask_img=None,
                       alpha=.05, verbose=False):
    """ Report the proportion of active voxels in input cluster.
    ...
    cluster_vals = stats[cluster]
    tdp = _true_positive_fraction(cluster_vals, hommel_value,
                                  alpha)
    return tdp
