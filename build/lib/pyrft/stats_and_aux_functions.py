...
def mvtstat(data):
    ...
    xbar = data.mean(n_dim) # Remember in python D is the last dimension of a D+1 array
...
def contrast_tstats(lat_data, design, contrast_matrix, check_error = 1):
    ...
    if check_error == 1:
        contrast_matrix, _, _ = contrast_error_checking(lat_data,design,contrast_matrix)
...
def contrast_error_checking(lat_data,design,contrast_matrix):
    ...
    if n_params != n_contrast_params:
        raise Exception('The dimensions of design and contrast_matrix do not match')
...
def contrast_tstats_noerrorchecking(lat_data, design, contrast_matrix):
    ...
    betahat = xtx_inv @ design.T @ lat_data.field.reshape( lat_data.fieldsize + (1,) )
